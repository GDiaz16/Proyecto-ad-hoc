...
class CU_distributed:
    ...
    def ADD(self,rx,sx):
        rx[0]= ALU().add(rx[0],sx[0])

    def DEC(self,rx,sx):
        rx[0]=ALU().substract(rx[0],sx[0])

    def MUL(self,rx,sx):
        rx[0]=ALU().multiply(rx[0],sx[0])

    def DIV(self,rx,sx):
        rx[0]=ALU().divide(rx[0],sx[0])

    def MOV(self,rx,int):
        rx[0]=int

    def LESS(self,rx,sx,label):
        if ALU().NOT(ALU().substract(rx[0],sx[0])<0):
            self.GOTO(label)
